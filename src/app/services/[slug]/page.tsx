...
export default function ServiceDetailPage() {
  ...
  if (!service) return <><PublicHeader /><main className="py-20 text-center"><h1 className="text-4xl font-bold mb-4 dark:text-white">Service Not Found</h1><Link href="/services" className="text-blue-600 hover:underline">← Back to Services</Link></main><PublicFooter /></>;
      ...
      <main className="py-12">
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          {service.features && service.features.length > 0 && (
            <div className="mb-8"><h2 className="text-2xl font-bold mb-4 dark:text-white">Features</h2><ul className="grid grid-cols-1 md:grid-cols-2 gap-3">{service.features.map((feature: string, i: number) => <li key={i} className="flex items-center gap-2 text-gray-700 dark:text-gray-300"><CheckCircle className="w-5 h-5 text-green-500" /> {feature}</li>)}</ul></div>
          )}
          <Link href="/contact" className="inline-flex items-center gap-2 bg-blue-600 text-white px-8 py-3 rounded-xl text-lg font-medium hover:bg-blue-700">Get This Service</Link>
        </div>
      </main>
      <PublicFooter />
    </>
