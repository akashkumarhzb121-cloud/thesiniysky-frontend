...
function formatContent(text) {
  ...
  return lines.map(function(line, i) {
    ...
    if (trimmed.startsWith('http')) return <a key={i} href={trimmed} target="_blank" className="text-blue-600 hover:underline">{trimmed}</a>;
    var withBold = trimmed.replace(/\*\*(.+?)\*\*/g, '<strong className="font-bold">\</strong>');
    if (trimmed.match(/^[A-Z\s]{4,}$/)) return <h3 key={i} className="font-bold text-xl mt-4 mb-2">{withBold}</h3>;
    if (trimmed.match(/^[-*]\s/)) return <li key={i} className="ml-4 mb-1">{withBold.replace(/^[-*]\s/, '')}</li>;
    return <p key={i} className="mb-1">{withBold}</p>;
  });
}
...
export default function BlogDetailPage() {
  ...
  if (!blog) return <><PublicHeader /><main className="py-20 text-center"><h1 className="text-4xl font-bold dark:text-white">Blog Not Found</h1><Link href="/blog" className="text-blue-600 hover:underline">Back to Blog</Link></main><PublicFooter /></>;
  ...
  return (
    ...
    <main className="py-12">
      <article className="container mx-auto px-4 max-w-4xl">
        ...
        <div className="text-gray-700 dark:text-gray-300 leading-relaxed text-lg ">{formatContent(blog.content || blog.excerpt)}</div>
      </article>
    </main>
    <PublicFooter /></>
  );
